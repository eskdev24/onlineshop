    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" step="0.01" name="price" value="{{ old('price', $product->getRawOriginal('price')) }}" required class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 p-2 border">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Discount Price ({{ settings('currency_symbol', '₵') }})</label>
                        <input type="number" step="0.01" name="discount_price" value="{{ old('discount_price', $product->getRawOriginal('discount_price')) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 p-2 border">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Cost Price ({{ settings('currency_symbol', '₵') }})</label>
                        <input type="number" step="0.01" name="cost_price" value="{{ old('cost_price', $product->getRawOriginal('cost_price')) }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 p-2 border">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Stock Quantity *</label>
                        <input type="number" name="stock_quantity" value="{{ old('stock_quantity', $product->stock_quantity) }}" required class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 p-2 border">
                    </div>
                </div>
            </div>

            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4" x-data="{ specs: {{ isset($product) && $product->specifications ? json_encode(array_map(fn($v, $k) => ['key' => $k, 'value' => $v], $product->specifications, array_keys($product->specifications))) : '[]' }}, addSpec() { this.specs.push({ key: '', value: '' }); }, removeSpec(index) { this.specs.splice(index, 1); } }">
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <div>
                    <label class="flex items-center space-x-2">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $product->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <p class="text-xs text-gray-400 mt-2">Main featured image. Leave blank to keep current.</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <div x-data="{ inputs: [1] }">
                    ...
                    <button type="button" @click="inputs.push(1)" class="text-xs font-medium text-indigo-600 hover:text-indigo-700 flex items-center">
                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                        Add another image
                    </button>
                </div>
                <p class="text-xs text-gray-400 mt-2">Upload additional images for the product gallery.</p>
            </div>
